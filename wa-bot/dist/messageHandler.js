...
Gunakan /groups untuk lihat JID yang valid.`
    ...
    async handlePendingResponse(from, text, message) {
        if (!this.pendingState)
            return false;
...
_0/BACK untuk kembali | /skip untuk lanjut | CANCEL untuk batal_`
        ...
        if (this.pendingState.state === 'draft_pending') {
            ...
            switch (cmd.action) {
                ...
                case 'po':
                    ...
                    draftCommands_1.PO_TYPES.forEach((type, index) => {
                        poMenu += `${index + 1}. *${type}*\n`;
                    });
                    poMenu += '\n---\n_0/BACK untuk kembali | CANCEL untuk batal_';
        ...
        if (this.pendingState.state === 'po_selection') {
            ...
            if (!isNaN(selection) && selection >= 1 && selection <= draftCommands_1.PO_TYPES.length) {
                const selectedPoType = draftCommands_1.PO_TYPES[selection - 1];
                ...
                if (mediaPaths && mediaPaths.length > 0 && fs_1.default.existsSync(mediaPaths[0])) {
                    await this.sock.sendMessage(from, {
                        image: { url: mediaPaths[0] },
                        caption: (0, draftCommands_1.formatDraftBubble)(draft || '', 'updated'),
                    });
                }
                else {
                    await this.sock.sendMessage(from, {
                        text: (0, draftCommands_1.formatDraftBubble)(draft || '', 'updated'),
                    });
                }
                await this.sock.sendMessage(from, {
                    text: (0, draftCommands_1.getDraftMenu)({ showCover: true, showLinks: true, showRegen: true, showSchedule: true, showBack: true }),
            ...
            // Invalid selection
            await this.sock.sendMessage(from, {
                text: `⚠️ Pilih angka *1-${draftCommands_1.PO_TYPES.length}* untuk tipe PO.\n\n_0/BACK untuk kembali | CANCEL untuk batal_`
            });
            return true;
        }
        ...
        if (this.pendingState.state === 'awaiting_edited_text') {
            ...
            if (mediaPaths && mediaPaths.length > 0 && fs_1.default.existsSync(mediaPaths[0])) {
                await this.sock.sendMessage(from, {
                    image: { url: mediaPaths[0] },
                    caption: (0, draftCommands_1.formatDraftBubble)(editedDraft, 'broadcast'),
                });
            }
            else {
                await this.sock.sendMessage(from, {
                    text: (0, draftCommands_1.formatDraftBubble)(editedDraft, 'broadcast'),
                });
            }
            // Show confirmation menu - user can YES to send, EDIT to edit again, etc
            await this.sock.sendMessage(from, {
                text: (0, draftCommands_1.getDraftMenu)({ showCover: false, showLinks: false, showRegen: false, showSchedule: true, showBack: false }),
            });
            return true;
        }
        return false;
